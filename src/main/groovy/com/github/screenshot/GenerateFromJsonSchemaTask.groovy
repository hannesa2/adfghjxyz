...
abstract class GenerateFromJsonSchemaTask extends DefaultTask{
    ...
    @TaskAction
    def generate(){
        def extension = project.getExtensions().getByType(ScreenshotExtension)
        if (project.plugins.hasPlugin('java')) {
            project.sourceSets.main.java.srcDirs targetDirectory
        }
        sourceFiles.each { it.mkdir() }
        
        def jS2PConfig = new ScreenshotConfig(extension, extension.executions[execution], targetDirectory, sourceFiles)
